...
The app reads `Blocks`, `Sessions`, and `Exercises` for v1. `Templates` is included so the spreadsheet can become the planning workspace without changing the app schema later. Completion/checkoff data is stored in Supabase, not written back to the sheet.
